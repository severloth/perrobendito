...
export default async function NewServicePage() {
  ...
  return (
    <FormShell title="Nuevo servicio" backHref="/admin/marketing">
      <form action={createService} className="flex flex-col gap-5">
        ...
        <Field label="Slug (URL, se genera solo si lo dejás vacío)" htmlFor="slug">
          <input id="slug" name="slug" placeholder="ej: identidad-visual" className={inputClass} />
        </Field>
        <Field label="Etiqueta" htmlFor="kicker">
          <input id="kicker" name="kicker" defaultValue="Servicio" className={inputClass} />
        </Field>
        ...
        <div className="grid grid-cols-3 gap-4">
          <Field label="Alcance" htmlFor="scope">
            <input id="scope" name="scope" className={inputClass} />
          </Field>
          ...
          <Field label="Formato" htmlFor="format">
            <input id="format" name="format" className={inputClass} />
          </Field>
        </div>
